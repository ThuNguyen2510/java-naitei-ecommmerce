	...
	public List<Product> loadHotTrendProduct(int gender) {
		...
		List<Object[]> objects = (List<Object[]>) getSession().createNativeQuery(
				"SELECT products.id, products.name, products.description, products.image, products.quantity, products.price, products.category_id, products.rate_avg "
						...
						+ "AND products.quantity > 0 " + "AND products.delete_time is null) "
						+ "GROUP BY orderitems.product_id " + "HAVING orderitems.product_id " + "IN (SELECT id "
						+ "FROM tripletclothes.products " + "WHERE image LIKE '%image-"
						+ ((gender < 10) ? "0" + gender : gender) + "-%') "
						+ "AND SUM(orderitems.quantity) > (SELECT AVG(quantity) "
						+ "FROM (SELECT SUM(quantity) AS 'quantity' " + "FROM orderitems " + "GROUP BY product_id) "
						+ "AS t) " + "ORDER BY products.rate_avg DESC, sum(orderitems.quantity) DESC " + "LIMIT 10")
				.getResultList();
...
//			p.setCategory(getSession().createQuery("from Category where id =" ).getResultList());
	...
	public List<Product> loadProductWithCategoryId(int categoryId, int productId) {
		if (productId != Constant.NONE_PRODUCT_ID) {
			return getSession()
					.createQuery("FROM Product WHERE id!=:productId AND category_id=:categoryId ORDER BY rate_avg DESC")
					.setParameter("productId", productId).setParameter("categoryId", categoryId).setMaxResults(4)
					.getResultList();
	...
	public List<Object> loadProductWithFilter(int categoryId, String productName, Integer fromprice, Integer toprice,
		...
		if (categoryId != Constant.ROOT_PARENID) {
			Predicate imageRestriction = builder.like(root.get("image"),
					"%-" + ((categoryId < 10) ? "0" + categoryId : categoryId) + "-%");
		...
		objects.add(getSession().createQuery(cr).setFirstResult((page - 1) * 16).setMaxResults(16).getResultList());
